...
public class TBMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (anim.GetInteger("up") > 2)
        {
            ...
            anim.SetFloat("walk", upParam + Input.GetAxis("Vertical"));
            
            //TURN           
            anim.SetFloat("turn", rightParam + Input.GetAxis("Horizontal"));
            //anim.SetFloat("turn", Input.GetAxis("Horizontal"));
            Debug.Log(anim.GetFloat("walk")+"/"+grounded+"/"+active);
            //walkの判定を消した、理由：回転しなかったため

            if (grounded && active){
                //if (anim.GetFloat("walk") !=0f && grounded && active){
                Debug.Log("test");
                trans.Rotate(0f, rightParam + Input.GetAxis("Horizontal") * 5f, 0f); //回転の速さ
                //trans.Rotate(0f, Input.GetAxis("Horizontal") * 5f, 0f); 
                
            }else{
                Debug.Log("test2");
            }
                           
            trans.Rotate(0f, anim.GetFloat("angle")* 0.2f, 0f);
    ...
    {             
        ...
        {
            //AIR MOVE
            if (!grounded && upParam + Input.GetAxis("Vertical") >  0.01f) rigidbo.AddForce(trans.forward * 0.05f, ForceMode.Impulse);
            ...
            //APLY THE SPEED
            if (grounded) rigidbo.velocity = forcedir * anim.GetFloat("speed") * -1f*4;
            //前に進むスピードが遅かったため４をかけた                
        }
    }
    ...
    public void OnDownStraightButton()
    {

        Debug.Log("上ボタンを押している");
        upParam = 1;
    }



    public void OnUpStraightButton()
    {
